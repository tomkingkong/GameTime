...
module.exports = class Game {
  ...
  newGame() {
    ...
    //Level Difficulty
    let difficulty = 10 * this.lvl;

    if (this.enemyWeaponCount === 1) {
      ...
      this.enemyTargets = [];
      this.batteries = [battery1, battery2, battery3];
      this.enemyWeaponCount = difficulty;
    }
  ...
  enemyShoot() {
    ...
    for (var i = 0; i < this.enemyWeaponCount * this.lvl; i++) {
      ...
      const enemyPos2 = { x: playerTargets[ this.randomPos(0, 8) ], y: 520 };
      const enemyTarget = 
  ...
  destructionDetection(array1, array2) {
    array1.forEach(target => {
      array2.forEach(weapon => {
        if ( target.isCollidingWith(weapon) ) {
          var deadTarget = array1.findIndex(target => target.x === weapon.p2.x);
          
          array1.splice(deadTarget, 1);
        }
      });
    });
  }
  ...
  detectCollisions() {    
    ...
    setTimeout(this.explosionDetection(), 10);
