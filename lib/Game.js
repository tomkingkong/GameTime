...
module.exports = class Game {
  ...
  findBattery(event) {
    ...
    var battMinX = batteries.map(battery => Math.abs(battery.x - mouseX));
    var indexOfBatt = battMinX.indexOf(Math.min(...battMinX));

    validBat = batteries[indexOfBatt];
    xPos = validBat.x + 30;
  ...
  shoot(event) {
    const {batteries, targets, missiles, playerMissileCount} = this;
    ...
    this.playerMissileCount = batteries.reduce((accumulator, battery) => {
      return accumulator + battery.missileCount;
    }, 0);
  }
  ...
  enemyShoot() {
    for (var i = 0; i < this.enemyWeaponCount * this.lvl; i++) {
      ...
      const enemyPos1 = { x: this.randomPos(10, 590), y: this.randomPos(-500, -100) };
