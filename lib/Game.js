...
module.exports = class Game {
  ...
  newGame() {
    ...
    //Create Batteries
    const battery1 = new Battery(0, 485, 60, 60, 'rgb(255, 255, 0)', 0, 0, 0, false, 10);
    const battery2 = new Battery(270, 490, 70, 60, 'rgb(255, 255, 0)', 0, 0, 0, false, 10);
    const battery3 = new Battery(540, 495, 60, 60, 'rgb(255, 255, 0)', 0, 0, 0, false, 10);
    
    if (this.lvl === 1) {
      this.cities = [eureka, sanFrancisco, sanLuisObispo, santaBarbara, losAngelas, sanDiego];
      this.batteries = [battery1, battery2, battery3];
      this.playerCount = this.batteries.reduce((accumulator, battery) => {
  ...
  objectiveDetection(weapons) {
    weapons.forEach((weapon) => {
      ...
      if ( posDiffY < 5 ) {
        weapons.splice(weapons.findIndex(missile => missile), 1);
  ...
  destructionDetection(targets, weapons) {
    targets.forEach((target) => {
      weapons.forEach((weapon) => {
        if ( weapon.isCollidingWith(target) ) {
          const deadTarget = targets.findIndex(target => 
            target === targets.find(target => 
              target.x === weapon.p2.x));
