...
module.exports = class Game {
  ...
  enemyShoot() {
    const {enemyWeaponCount, enemyMissiles, lvl, randomPos} = this;
  ...
  explosionDetection(targets) {
    targets.forEach(target => {
      this.enemyMissiles.forEach(weapon => {
        let hypoDiff = Math.hypot(target.x - weapon.x, target.y - weapon.y);
        let deadWeapon = this.enemyMissiles.indexOf(weapon);
